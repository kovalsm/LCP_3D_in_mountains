		...
		try (Scanner scanner = new Scanner(suborPLY)) {
			...
			while (!riadok.equals("end_header")) {
				...
				if (riadok.startsWith("property ")) {
					...
					switch (riadok) {
					case "property double x":
						pocetKX++;
						stlpecSuradniceX = pocetKX;
						break;
					case "property double y":
						pocetKY++;
						stlpecSuradniceY = pocetKY;
						break;
					case "property double z":
						pocetKZ++;
						stlpecSuradniceZ = pocetKZ;
						break;
					case "property uchar red":
						pocetKR++;
						stlpecFarbaR = pocetKR;
						break;
					case "property uchar green":
						pocetKG++;
						stlpecFarbaG = pocetKG;
						break;
					case "property uchar blue":
						pocetKB++;
						stlpecFarbaB = pocetKB;
						break;
			...
			if (stlpecFarbaG == -1 && stlpecFarbaB == -1 && stlpecFarbaR == -1) {
				suFarby = false;
			} else {
				if (stlpecFarbaB != -1 && stlpecFarbaG != -1 && stlpecFarbaR != -1) {
					suFarby = true;
				} else {
					// nie je pripad ze su vsetky alebo ziadna
					return null;
			...
			for (int i = 0; i < pocetVrcholov; i++) {
				...
				double suradnicaX = Double.parseDouble(hodnoty[stlpecSuradniceX - 1]);
				double suradnicaY = Double.parseDouble(hodnoty[stlpecSuradniceY - 1]);
				double suradnicaZ = Double.parseDouble(hodnoty[stlpecSuradniceZ - 1]);
				int farbaR = Integer.parseInt(hodnoty[stlpecFarbaR - 1]);
				int farbaG = Integer.parseInt(hodnoty[stlpecFarbaG - 1]);
				int farbaB = Integer.parseInt(hodnoty[stlpecFarbaB - 1]);
				...
				if (suNormaly) {
					...
					vrcholy[i] = new Vertex(i, riadok, suradnicaX, suradnicaY, suradnicaZ, farbaR, farbaG, farbaB, suradnicaXNormaly,
							suradnicaYNormaly, suradnicaZNormaly);
				} else {
					vrcholy[i] = new Vertex(i, riadok, suradnicaX, suradnicaY, suradnicaZ, farbaR, farbaG, farbaB, 0, 0, 0);
				}
